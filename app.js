...
const userInput = document.querySelector("#reserve [name=\"user\"]");
...
const accounts = {
  gestionnaire: { name: "Gestionnaire", role: "gestionnaire" },
  stock: { name: "Stock", role: "stock" },
  utilisateur: { name: "Utilisateur", role: "utilisateur" },
};
...
function renderReservations() {
  ...
  const reservations = appState.role === "utilisateur"
    ? appState.reservations.filter((item) => item.user === appState.user?.name)
    : appState.reservations;
  ...
  if (!reservations.length) {
    reservationsTable.innerHTML = `
      <tr>
        <td colspan="4" class="muted">Aucune réservation à afficher.</td>
      </tr>
    `;
  }
}
...
function render() {
  ...
  if (userInput) {
    userInput.value = appState.user?.name || "";
    userInput.readOnly = appState.role === "utilisateur";
  }
  renderInventory();
  renderCatalog();
  renderAvailable();
