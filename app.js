  ...
  if (assignButton) {
    ...
    if (accessory) {
      ...
      if (accessory.assignedChairId) {
        ...
        if (chair) {
          if (chair.state !== "utilise") {
            alert("Le fauteuil doit être en état utilisé pour lier un accessoire.");
            return;
          }
          accessory.state = chair.state;
          logEvent(`Stock a lié l'accessoire ${accessory.id} au fauteuil ${chair.id}.`);
        }
      } else {
        accessory.state = "disponible";
        logEvent(`Stock a délié l'accessoire ${accessory.id}.`);
      }
      render();
    }
    return;
