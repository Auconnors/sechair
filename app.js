...
function renderAccessories() {
  const isManager = appState.role === "gestionnaire";
  const isStock = appState.role === "stock";
  const isReserveOperator = appState.role === "utilisateur";
...
function handleAccessoryAction(event) {
  ...
  if (assignButton) {
    if (!["stock", "gestionnaire", "utilisateur"].includes(appState.role)) {
      alert("Seul le stock, la réserve ou le gestionnaire peut lier un accessoire.");
      return;
    }
  ...
  if (quickButton) {
    if (!["stock", "utilisateur"].includes(appState.role)) {
      alert("Seul le stock ou la réserve peut modifier l'état des accessoires.");
      return;
    }
