...
const linkAccessorySelect = document.querySelector("#link-accessory [name=\"accessory\"]");
...
function addAccessory({ type, prefix }) {
  const id = formatAccessoryId(prefix || "ACC-");
  appState.accessories.push({ id, type, state: "disponible", assignedChairId: "" });
}
...
function renderCatalog() {
  ...
  if (linkAccessorySelect) {
    linkAccessorySelect.innerHTML = `
      <option value="">Choisir</option>
      ${appState.accessories
        .map((accessory) => `<option value="${accessory.id}">${accessory.id} — ${accessory.type}</option>`)
        .join("")}
    `;
  }
}
...
function handleAccessoryAction(event) {
  ...
  if (assignButton) {
    if (appState.role !== "stock") {
      alert("Seul le stock peut lier un accessoire.");
      return;
    }
...
function handleLinkAccessorySubmit(event) {
  ...
  const accessorySelect = formData.get("accessory");
  const chairId = (formData.get("chairId") || "").trim();
  const id = accessoryId || accessorySelect;
