...
const managerAddTypeSelect = document.querySelector("#manager-search [name=\"addType\"]");
const managerAddModelSelect = document.querySelector("#manager-search [name=\"addModel\"]");
const managerAddSizeSelect = document.querySelector("#manager-search [name=\"addSize\"]");
...
function renderInventory() {
  ...
  if (!filtered.length) {
    inventoryTable.innerHTML = `
      <tr>
        <td colspan="7" class="muted">Aucun fauteuil pour ces filtres.</td>
      </tr>
    `;
  }
}
...
function renderCatalog() {
  ...
  if (managerAddTypeSelect) {
    managerAddTypeSelect.innerHTML = `
      <option value="">Choisir</option>
      ${appState.catalog.types.map((type) => `<option>${type}</option>`).join("")}
    `;
  }
  if (managerAddModelSelect) {
    managerAddModelSelect.innerHTML = `
      <option value="">Choisir</option>
      ${appState.catalog.models.map((model) => `<option>${model}</option>`).join("")}
    `;
  }
  if (managerAddSizeSelect) {
    managerAddSizeSelect.innerHTML = `
      <option value="">Choisir</option>
      ${appState.catalog.sizes.map((size) => `<option>${size}</option>`).join("")}
    `;
  }
...
function handleManagerSearchSubmit(event) {
  ...
  if (action === "add-chair") {
    const payload = {
      type: ((formData.get("newType") || "").trim() || (formData.get("addType") || "").trim()),
      model: ((formData.get("newModel") || "").trim() || (formData.get("addModel") || "").trim()),
      size: (((formData.get("newSize") || "").trim() || (formData.get("addSize") || "").trim()).toUpperCase()),
      chairId: (formData.get("chairId") || "").trim(),
    };

    if (!payload.type || !payload.model || !payload.size) {
      alert("Type, modèle et taille sont requis (existants ou nouveaux) pour ajouter un fauteuil.");
      return;
    }
    ...
    addChairs(payload);
    ["addType", "newType", "addModel", "newModel", "addSize", "newSize", "chairId"].forEach((name) => {
