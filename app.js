...
const reserveAccessoryChairInput = document.querySelector("#link-accessory-reserve [name=\"chairId\"]");
const reserveAccessoryChairLabel = document.querySelector("#link-accessory-reserve [name=\"chairLabel\"]");
...
const searchTypeSelect = document.querySelector("#search [name=\"type\"]");
...
function addAccessory({ type, size, accessoryId }) {
  const id = accessoryId || formatAccessoryId("ACC-");
  const accessory = { id, type, size: (size || "").toUpperCase(), state: "disponible", assignedChairId: "", history: [] };
...
function renderStats() {
  ...
  const byState = {
    disponible: appState.chairs.filter((item) => item.state === "disponible").map((item) => item.id),
    reserve: appState.chairs.filter((item) => item.state === "reserve").map((item) => item.id),
    rendu: appState.chairs.filter((item) => item.state === "rendu").map((item) => item.id),
    utilise: appState.chairs.filter((item) => item.state === "utilise").map((item) => item.id),
  };

  if (appState.role === "gestionnaire") {
    const rows = [
      { label: "Stock total fauteuils", value: String(total) },
      { label: "En réserve", value: byState.disponible.join(", ") || "—" },
      { label: "En réservé", value: byState.reserve.join(", ") || "—" },
      { label: "En réparation", value: byState.rendu.join(", ") || "—" },
      { label: "En prêt", value: byState.utilise.join(", ") || "—" },
    ];
    ...
    statsContainer.innerHTML = [
      ...
      { label: "En réserve", value: byState.disponible.length },
      { label: "Réservés", value: byState.reserve.length },
      { label: "En prêt", value: byState.utilise.length },
    ]
      .map(
        (stat) => `
          <div class="stat">
...
function renderAccessories() {
      ...
      const stockActions = `
        <div class="actions">
          ${accessory.state === "reserve"
            ...
            : ""}
          ${accessory.state === "utilise" && appState.role !== "pret"
  ...
  accessoriesTables.forEach((table) => {
    ...
    if (headRow) {
      headRow.innerHTML = isManager
        ? "<th>Identifiant</th><th>Type</th><th>Taille</th><th>Lié à</th><th>Actions</th>"
        : "<th>Identifiant</th><th>Type</th><th>Taille</th><th>État</th><th>Lié à</th><th>Actions</th>";
    }
    table.innerHTML = tableHtml;
    if (!filtered.length) {
      table.innerHTML = `
        <tr>
          <td colspan="${isManager ? 5 : 6}" class="muted">Aucun accessoire pour ces filtres.</td>
        </tr>
      `;
    }
  });
...
function renderAvailable() {
  const filtered = appState.chairs.filter((chair) => {
    ...
    if (appState.search.type && chair.type !== appState.search.type) return false;
    ...
    .map(
      (chair) => `
        <tr>
          ...
          <td>${chair.size}</td>
          <td><span class="badge" data-state="${chair.state}">${stateLabels[chair.state]}</span></td>
          <td><button class="button button-secondary" data-chair="${chair.id}" data-type="${chair.type || ""}" data-model="${chair.model}" data-size="${chair.size}" data-state="${chair.state}">Choisir</button></td>
        </tr>
      `,
    )
    .join("");
...
function renderReservations() {
  const reservations = appState.role === "utilisateur"
    ? appState.reservations
      .filter((item) => item.accountName === appState.user?.name && ["reserve", "utilise"].includes(item.state))
      .sort((a, b) => {
        const order = { reserve: 1, utilise: 2 };
        return (order[a.state] || 99) - (order[b.state] || 99);
      })
    : appState.role === "pret"
      ? appState.reservations
        .filter((item) => ["reserve", "utilise"].includes(item.state))
        .sort((a, b) => {
          const order = { reserve: 1, utilise: 2 };
          return (order[a.state] || 99) - (order[b.state] || 99);
        })
  ...
  reservationsTable.innerHTML = reservations
    .map((reservation) => {
      ...
      const chairLabel = chair
        ? `${chair.type || "—"} • ${chair.model || "—"} • ${chair.size || "—"}`
        : reservation.chairId;
      let actions = "—";
      if (["stock", "gestionnaire"].includes(appState.role) && stateTransitions[reservation.state]) {
        actions = `<button data-id="${reservation.id}">${actionLabel}</button>`;
      } else if (appState.role === "pret") {
        if (reservation.state === "reserve") {
          actions = `<button data-id="${reservation.id}" data-target-state="utilise">Passer en prêt</button>`;
        } else if (reservation.state === "utilise") {
          actions = `<button data-select-chair="${reservation.chairId}" data-chair-label="${chairLabel}">Choisir</button>`;
        }
      }
      return `
      <tr>
...
function handleAccessorySubmit(event) {
  ...
  addAccessory({ type, size, accessoryId });
  event.target.reset();
  render();
}
...
function handleReservationAction(event) {
  const selectButton = event.target.closest("button[data-select-chair]");
  if (selectButton) {
    const chairId = selectButton.dataset.selectChair;
    const label = selectButton.dataset.chairLabel || chairId;
    chairSelect.value = chairId;
    if (chairLabelInput) chairLabelInput.value = label;
    if (reserveAccessoryChairInput) reserveAccessoryChairInput.value = chairId;
    if (reserveAccessoryChairLabel) reserveAccessoryChairLabel.value = label;
    return;
  }
  ...
  if (!reservation) return;
  const target = button.dataset.targetState || stateTransitions[reservation.state];
...
function handleAvailableClick(event) {
  ...
  if (!button) return;
  const label = `${button.dataset.type || "Type"} • ${button.dataset.model || "Modèle"} • ${button.dataset.size || "Taille"}`;
  chairSelect.value = button.dataset.chair;
  if (chairLabelInput) {
    chairLabelInput.value = label;
  }
  if (reserveAccessoryChairInput) reserveAccessoryChairInput.value = button.dataset.chair;
  if (reserveAccessoryChairLabel) reserveAccessoryChairLabel.value = label;
}
